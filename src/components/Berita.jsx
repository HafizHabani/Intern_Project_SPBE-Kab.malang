      ...
      {newsDummy.map((news, index) => (
        <div key={index} className="card card-compact w-full sm:w-1/2 md:w-1/3 lg:w-1/4 bg-base-100 mt-2 md:mt-0 md:mr-4 lg:mr-0"> {/* Adjusted card width and added margin-right */}
          ...
          <div className="card-body">
            ...
            <p className='text-xs md:text-sm'>{news.description}</p> {/* Adjusted font size */}
            <div className="card-actions justify-between items-center">
              <div className='text-slate-500 text-xs md:text-sm'>{news.author}</div> {/* Adjusted font size */}
